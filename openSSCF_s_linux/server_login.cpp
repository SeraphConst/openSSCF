...
void file2request(const char * filepath, std::vector<char *> & reqList)
{
	...
	int strLen = contents.size();			//ºóÃæ»¹»áÓÃµœµÄÎÄŒþ³€¶È±äÁ¿

	infile.close();
	buffer.str("");			//ŸÝËµÕâÑùÊÇÇå¿Õstringstream¡£Á÷¶ÔÏó£¿



	//std::cout << contents;
	std::string msgLine;				//±šÎÄÄÚÈÝ
	int msgLineLen = 0;

	int msgLen = DEFAULT_BUFFER_LENGTH - 50;		//Ã¿ŽÎŽ«ÊämsgLenžö
				...
				if( strcmp("[EXE]", buf)==0 )
				{
					...
					else if( strcmp("REQ_GET", cmd_header.c_str())==0 )
					{
						if( isINV == false )
						{
							...
							file2request( "mingran3_menu.tri", backList );
